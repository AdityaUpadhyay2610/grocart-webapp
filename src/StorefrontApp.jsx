...
function AppShell({ categories = [] }) {
  ...
  return (
    <div className={`relative min-h-screen bg-background text-on-surface transition-colors duration-300 flex flex-col font-sans overflow-x-hidden select-none`}>
      ...
      {!showPaymentScreen && (
        <header className="fixed top-0 w-full z-50 bg-glass-fill backdrop-blur-xl shadow-[0_1px_8px_rgba(0,0,0,0.04)] border-b border-surface-variant/40">
          <div className="h-20 w-full max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-10 flex items-center justify-between gap-4 md:gap-8">
            ...
            <div className="flex items-center gap-4 sm:gap-6 shrink-0">
              ...
              <button 
                ...
                className="hidden md:flex items-center gap-1.5 text-on-surface-variant hover:text-primary transition-colors px-3.5 py-2 rounded-xl bg-surface-container-low border border-surface-variant/40 max-w-[280px] text-left cursor-pointer shadow-xs"
              >
            ...
            <div className="flex-1 max-w-2xl relative hidden md:block">
              <form onSubmit={handleSearchSubmit} className="relative flex items-center">
                ...
                <input 
                  ...
                  className="w-full bg-surface-container-lowest border border-surface-variant/40 h-12 pl-12 pr-10 rounded-xl shadow-inner focus:ring-2 focus:ring-primary/20 focus:outline-none transition-all font-body-md text-sm text-on-surface placeholder:text-slate-text/70"
                />
                {searchQuery && (
                  <button 
                    type="button" 
              ...
              {searchQuery && (
                <div className="absolute left-0 right-0 top-full mt-2 bg-surface-container-lowest border border-surface-variant/50 rounded-2xl shadow-2xl z-50 max-h-80 overflow-y-auto animate-fade-in divide-y divide-surface-variant/30 text-left">
                    ...
                    searchResults.slice(0, 8).map(item => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          {item.imageUrl && (
                            <img src={item.imageUrl} alt={item.itemName} className="w-9 h-9 rounded-lg object-cover bg-surface-container-low" />
                          )}
                          <div>
            ...
            <nav className="flex items-center gap-3 sm:gap-5">
              <button 
                onClick={() => navigate("/home")}
                className={`font-label-md text-sm transition-colors cursor-pointer hidden sm:block ${isHome ? 'text-primary font-extrabold' : 'text-on-surface-variant hover:text-primary'}`}
              >
                Home
              </button>
              ...
              <button 
                ...
                className={`font-label-md text-sm transition-colors cursor-pointer hidden sm:block ${isCategories ? 'text-primary font-extrabold' : 'text-on-surface-variant hover:text-primary'}`}
              >
                Categories
              </button>
              ...
              <button 
                ...
                className={`font-label-md text-sm transition-colors cursor-pointer hidden sm:block ${isOrders ? 'text-primary font-extrabold' : 'text-on-surface-variant hover:text-primary'}`}
              >
                Orders
              </button>
              ...
              <button 
                ...
                className="w-10 h-10 rounded-xl bg-surface-container-low border border-surface-variant/40 hover:border-primary text-on-surface flex items-center justify-center transition-all cursor-pointer shadow-xs"
              ...
              <div 
                ...
                className="relative cursor-pointer hover:scale-105 transition-transform flex items-center justify-center p-1.5 rounded-xl bg-surface-container-low border border-surface-variant/40"
                title="View Cart"
              >
                <span className="material-symbols-outlined text-on-surface-variant text-[24px]">shopping_cart</span>
                {cartCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 min-w-[20px] h-[20px] bg-primary text-on-primary text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white dark:border-slate-900 px-1 shadow-sm animate-scale-in">
                    {cartCount}
                  </span>
                )}
              </div>
              ...
              <div className="relative">
                ...
                {menuExpanded && (
                  <div className="absolute right-0 top-12 bg-surface-container-lowest border border-surface-variant/40 rounded-2xl p-2 w-52 shadow-2xl z-50 animate-fade-in text-left">
                    <div className="px-3 py-2 border-b border-surface-variant/30 mb-1">
                      <p className="text-xs font-bold text-on-surface truncate">{user?.username || "Guest User"}</p>
                    ...
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-xs font-bold text-on-surface hover:bg-surface-container-low rounded-xl transition-colors cursor-pointer flex items-center gap-2"
                    >
          ...
          <div className="md:hidden px-4 pb-3">
            <form onSubmit={handleSearchSubmit} className="relative flex items-center">
              ...
              <input 
                ...
                className="w-full bg-surface-container-lowest border border-surface-variant/40 h-10 pl-10 pr-8 rounded-xl text-xs text-on-surface"
              />
            </form>
          </div>
        </header>
      ...
      <footer className="w-full bg-surface-container-lowest border-t border-surface-variant/30 py-16 mt-16 text-left">
        <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-10 grid grid-cols-1 md:grid-cols-4 gap-10">
          ...
          <div className="flex flex-col gap-3">
            <h4 className="font-label-md text-on-surface uppercase tracking-wider text-xs font-bold">Company</h4>
          ...
          <div className="flex flex-col gap-3">
            <h4 className="font-label-md text-on-surface uppercase tracking-wider text-xs font-bold">Support</h4>
          ...
          <div className="flex flex-col gap-3">
            <h4 className="font-label-md text-on-surface uppercase tracking-wider text-xs font-bold">Connect</h4>
            <div className="flex gap-3">
              <a href="#" className="w-9 h-9 rounded-full bg-surface-container-low flex items-center justify-center hover:bg-primary hover:text-white transition-all text-on-surface-variant">
        ...
        <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-10 mt-12 pt-6 border-t border-surface-variant/20 flex flex-col sm:flex-row justify-between items-center gap-4 text-on-surface-variant text-xs">
      ...
      {showLocationModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white dark:bg-[#111724] rounded-3xl p-6 w-full max-w-md shadow-2xl border border-surface-variant/50 text-left">
            <div className="flex items-center justify-between pb-4 border-b border-surface-variant/30">
              <div className="flex items-center gap-2">
                <span className="material-symbols-outlined text-primary">location_on</span>
                <h3 className="font-headline-md text-lg font-black text-on-surface">Select Delivery Location</h3>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 max-h-56 overflow-y-auto pr-1">
                {(presetLocations || []).map((preset) => (
                  <button
                    ...
                    className="flex flex-col text-left p-3 rounded-xl border border-surface-variant/40 hover:border-primary hover:bg-primary-50/20 transition-all cursor-pointer"
                  >
                    <span className="text-xs font-bold text-on-surface">{preset.city}</span>
